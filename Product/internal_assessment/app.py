...
app.secret_key = 'you-will-never-guess'  # Define a secret key for the application.
...
token_key = 'TOKEN_ENCRYPTION_KEY'  # Retrieve the token encryption key from environment variables
...
@app.route('/follow_post/<int:post_id>', methods=['POST'])
@token_required
def follow_post(post_id):
    ...
    db = database_worker("frisbeecrew.db")
    check_like_query = f"SELECT * FROM following WHERE user_id = {user_id} AND post_id = {post_id}"
    following_record = db.run_fetchone(query=check_like_query)

    # count the number of times a post_id appears in the like table
    count_likes_query = f"SELECT COUNT(*) FROM following WHERE post_id = {post_id}"

    if following_record:
        # If the follow record exists, delete it (unfollow)
        unfollow_query = f"DELETE FROM following WHERE user_id = {user_id} AND post_id = {post_id}"
        db.run_save(query=unfollow_query)

    else:
        # If the like record doesn't exist, create it (like)
        following_query = f"INSERT INTO following (user_id, post_id) VALUES ('{user_id}', '{post_id}')"
        db.run_save(query=following_query)

    return redirect('/post/' + str(post_id))


@app.route('/mutual_signups/<int:post_id>', methods=['GET'])
@token_required
def mutual_signups(post_id):
    current_user = jwt.decode(session['token'], token_key, algorithms=['HS256'])
    current_user_id = current_user['user_id']
    db = database_worker("frisbeecrew.db")

    # Fetch all users except the current user
    all_users_query = f"SELECT id, name FROM users WHERE id != {current_user_id}"
    all_users = db.search(query=all_users_query)

    mutual_signups_result = {}

    # Fetch events signed up by the current user for the specific post_id
    current_user_signups_query = f"""
        SELECT post_id 
        FROM signups 
        WHERE user_id = {current_user_id}
          AND post_id = {post_id}
    """
    current_user_signups = db.search(query=current_user_signups_query)

    # Convert current user signups to a set for easy comparison
    current_user_signup_ids = {signup[0] for signup in current_user_signups}

    # Outer loop through each user
    for user in all_users:
        other_user_id = user[0]
        other_user_name = user[1]

        # Fetch events signed up by the other user for the given post_id
        other_user_signups_query = f"""
            SELECT post_id 
            FROM signups 
        ...
        # Check if the other user signed up for the event
        other_user_signed_up = 1 if other_user_signups else 0

        # Store result for each user
        mutual_signups_result[other_user_name] = other_user_signed_up

    return render_template('mutual_signup.html', mutual_signups=mutual_signups_result, current_user_id=current_user_id)
...
def profile():
    ...
    events_signed_up = db.search(f"SELECT posts.* FROM signups INNER JOIN posts ON signups.post_id = posts.id WHERE signups.user_id = {user_id}")
    return render_template('profile.html', posts=posts, comments=comments, liked_posts=liked_posts, events_signed_up=events_signed_up, user=user[0])
...
def view_post(post_id):
    ...
    user_follow = db.run_fetchone(f"SELECT * FROM following WHERE post_id = {post_id} AND user_id = {current_user['user_id']}")
    user_signup = db.run_fetchone(f"SELECT * FROM signups WHERE post_id = {post_id} AND user_id = {current_user['user_id']}")
    is_liked = user_like is not None
    is_followed = user_follow is not None
    signed_up = user_signup is not None

    if post:
        return render_template('post.html', post=post[0], comments=comments, is_liked=is_liked, is_followed=is_followed, signed_up=signed_up)
...
def create_post():
    ...
    if request.method == 'POST':
        ...
        create_post = f"INSERT INTO posts (title, category, content, datetime, image_name, user_id, event_date, event_time) VALUES ('{title}', '{category}', '{content}', '{datetime.now()}', '{filename}','{user_id}', '{event_date}','{event_time}')"
        db.run_save(query=create_post)
        test_query = f"SELECT * FROM posts"
        print(db.search(query=test_query))
...
def following():
    ...
    user = db.search(f"SELECT * FROM users WHERE id = {user_id}")
    comments = db.search(f"SELECT * FROM comments WHERE user_id = {user_id} ORDER BY comments.datetime DESC")
    followed_posts = db.search(
        f"SELECT posts.* FROM following INNER JOIN posts ON following.post_id = posts.id WHERE following.user_id = {user_id}")
    return render_template('following.html', comments=comments, followed_posts=followed_posts, user=user[0])


@app.route('/pickup_game')
@token_required
def pickup_game():
    db = database_worker("frisbeecrew.db")
    posts = db.search("SELECT posts.*, users.name FROM posts INNER JOIN users ON posts.user_id = users.id WHERE posts.category = 'Pickup game' ORDER BY posts.datetime DESC")
    return render_template('pickup_game.html', posts=posts)
...
def tournament():
    db = database_worker("frisbeecrew.db")
    posts = db.search("SELECT posts.*, users.name FROM posts INNER JOIN users ON posts.user_id = users.id WHERE posts.category = 'Tournament' ORDER BY posts.datetime DESC")
    return render_template('tournament.html', posts=posts)
...
def disk_golf():
    db = database_worker("frisbeecrew.db")
    posts = db.search("SELECT posts.*, users.name FROM posts INNER JOIN users ON posts.user_id = users.id WHERE posts.category = 'Disk golf' ORDER BY posts.datetime DESC")
    return render_template('disk_golf.html', posts=posts)


@app.route('/beach_frisbee')
@token_required
def beach_frisbee():
    db = database_worker("frisbeecrew.db")
    posts = db.search("SELECT posts.*, users.name FROM posts INNER JOIN users ON posts.user_id = users.id WHERE posts.category = 'Beach frisbee' ORDER BY posts.datetime DESC")
    return render_template('beach_frisbee.html', posts=posts)
...
def login():
    if request.method == "POST":
        ...
        if len(email) > 0 and len(password) > 0:
            ...
            if user:
                # Unpack user data
                id, name, username, email, hash = user[0]
...
def register():
    ...
    if request.method == "POST":
        ...
        if len(name) > 0 and len(username) > 0 and len(email) > 0 and len(password) > 0:
            if password != password_confirm:
                ...
                register = f"INSERT into users(name, username, email, password) values('{name}', '{username}', '{email}', '{encrypt_password(password)}')"
...
def send_message(receiver_id):
    ...
    if request.method == 'POST':
        ...
        db = database_worker("frisbeecrew.db")
        query = f"INSERT INTO messages (sender_id, receiver_id, content) VALUES ({user_id}, {receiver_id}, '{content}')"
        db.run_save(query=query)

        flash(('Message sent successfully!', 'success'))
        return redirect(url_for('inbox'))

    # Get the receiver's details to display on the message form
    db = database_worker("frisbeecrew.db")
    receiver = db.search(f"SELECT * FROM users WHERE id = {receiver_id}")

    return render_template('send_message.html', receiver=receiver[0])
...
def inbox():
    ...
    db = database_worker("frisbeecrew.db")
    users = db.search(f"""SELECT name, id FROM users WHERE id != {user_id}""")
    ...
    return render_template('inbox.html', users=users, messages=messages)


@app.route('/signup/<int:post_id>', methods=['GET', 'POST'])
@token_required
def signup(post_id):
    current_user = jwt.decode(session['token'], token_key, algorithms=['HS256'])
    user_id = current_user['user_id']
    ...
    db = database_worker("frisbeecrew.db")
    check_signup_query = f"SELECT * FROM signups WHERE user_id = {user_id} AND post_id = {post_id}"
    signup_record = db.run_fetchone(query=check_signup_query)

    # count the number of times a post_id appears in the like table
    count_signups_query = f"SELECT COUNT(*) FROM signups WHERE post_id = {post_id}"

    if signup_record:
        # If the like record exists, delete it (unlike)
        delete_signup_query = f"DELETE FROM signups WHERE user_id = {user_id} AND post_id = {post_id}"
        db.run_save(query=delete_signup_query)

    else:
        # If the like record doesn't exist, create it (like)
        signup_query = f"INSERT INTO signups (user_id, post_id) VALUES ('{user_id}', '{post_id}')"
        db.run_save(query=signup_query)
...
def calendar():
    db = database_worker("frisbeecrew.db")
    event_query = f"""SELECT id, title, event_date, event_time FROM posts ORDER BY event_date"""
    events = db.search(query=event_query)
